#Updated 03/01/2025
#Does not require privilege mode
import whois
import dns.resolver
import nmap
import requests
...
# Function to perform port scanning without privileged actions
def port_scan(target):
    try:
        nm = nmap.PortScanner()
        # Perform a non-privileged TCP connect scan (-sT) without OS detection or version detection
        nm.scan(target, arguments='-p 1-1024 -sT --open')
...
def subdomain_enumeration(domain):
    ...
    try:
        ...
        wordlist = [
            ...
            "shop", "support", "web", "portal", "app", "secure", "static"
        ]
